...
const AdminPage = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "alle" | "godkjent" | "avslått" | "innsendt"
  >("alle");
  const [typeFilter, setTypeFilter] = useState<
    "alle" | "okonomi" | "tillatelse"
  >("alle");
  ...
  const filteredApplications = applications.filter((app) => {
    const matchesStatus =
      statusFilter === "alle" || app.status === statusFilter;
  ...
  const mappedApplications = filteredApplications.map((app) => ({
    ...
    description: app.firma
      ? `Firma: ${app.firma} - Søknad ID: ${app._id}`
      : `Ingen firma oppgitt - Søknad ID: ${app._id}`,
...
<div className="flex justify-between items-center mb-8">
  ...
  <Link href="/adminpage/id" className={buttonVariants()}>
    Se alle søknadsIdér
  </Link>
</div>
      ...
      <div className="mb-6">
        ...
        <h2 className="text-xl font-semibold mt-4">
          Totalsum av godkjente økonomiske søknader
        </h2>
        <p>{formatNumber(approvedEconomyTotal)} kr</p>
        <h2 className="text-xl font-semibold mt-4">
          Totalsum av avviste økonomiske søknader
        </h2>
      ...
      <div className="mb-4 flex gap-4">
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setStatusFilter(
                e.target.value as "alle" | "godkjent" | "avslått" | "innsendt"
              )
            }
        ...
        <div>
          ...
          <select
            value={typeFilter}
            onChange={(e) =>
              setTypeFilter(e.target.value as "alle" | "okonomi" | "tillatelse")
            }
            className="border px-2 py-1 rounded"
          >
            <option value="alle">Alle</option>
      ...
      <MaxWidthWrapper>
        ...
        <ChartComponent
          applicationData={applicationTypes}
