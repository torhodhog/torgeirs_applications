...
const ApplicationDetailPage = () => {
  ...
  useEffect(() => {
    const fetchApplication = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            `Søknad ikke funnet eller annen feil. Status: ${res.status}`
          );
        }
  ...
  const handleStatusUpdate = async (newStatus: string) => {
    try {
      const response = await fetch(`/api/soknad/${id}`, {
        // Oppdater URL til `/api/soknad/${id}`
        method: "PATCH",
      ...
      if (!response.ok) {
        throw new Error(
          `Feil ved oppdatering av søknadsstatus. Status: ${response.status}`
        );
      }
  ...
  return (
    <div className="application-detail max-w-xl mx-auto mt-20 p-6 border rounded-lg shadow-lg">
      ...
      <Link
        href="/adminpage"
        className="text-blue-500 hover:text-blue-700 flex items-center mb-4 no-print"
      >
        <svg
      ...
      <p style={{ marginBottom: "10px" }}>
        <strong>Navn:</strong> {application.navn}
      </p>
      <p style={{ marginBottom: "10px" }}>
        <strong>ID:</strong>
        {application._id}
      ...
      <p style={{ marginBottom: "10px" }}>
        <strong>Type ID:</strong> {application.type_id}
      </p>
      <p style={{ marginBottom: "10px" }}>
        <strong>E-post:</strong> {application.epost}
      </p>
      <p style={{ marginBottom: "10px" }}>
        <strong>Beskrivelse:</strong> {application.beskrivelse}
      </p>
      <p style={{ marginBottom: "10px" }}>
        <strong>Type:</strong> {application.soknadstype}
      </p>
      {application.firma && (
        <p style={{ marginBottom: "10px" }}>
          <strong>Firma:</strong> {application.firma}
        </p>
      )}
      {application.belop && (
        <p style={{ marginBottom: "10px" }}>
          <strong>Beløp:</strong> {application.belop} kr
        </p>
      )}
      {application.kontonummer && (
        <p style={{ marginBottom: "10px" }}>
          <strong>Kontonummer:</strong> {application.kontonummer}
        </p>
      )}
      {application.tillatelsestype && (
        <p style={{ marginBottom: "10px" }}>
          <strong>Tillatelsestype:</strong> {application.tillatelsestype}
        </p>
      )}
      {application.begrunnelse && (
        <p style={{ marginBottom: "10px" }}>
          <strong>Begrunnelse:</strong> {application.begrunnelse}
        </p>
      )}
      {application.status && (
        <p style={{ marginBottom: "10px" }}>
      ...
      {application.feedback && (
        <p style={{ marginBottom: "10px" }}>
          <strong>Tilbakemelding:</strong> {application.feedback}
        </p>
      )}
